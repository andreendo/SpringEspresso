...
public class ProjetoSeeder {
    private static final Logger logger = LoggerFactory.getLogger(ProjetoSeeder.class);
    ...
    public void seedProjetos() {
        if (projetoRepository.count() > 0) {
            logger.info("✓ Projetos já existem no banco de dados.");
            return;
        }

        logger.info("Criando projetos iniciais...");
        ...
        if (admin == null || tester == null) {
            logger.info("X Falha ao criar projetos: usuários 'admin' ou 'tester' não encontrados.");
            return;
        }
        ...
        logger.info("✓ Projetos iniciais criados com sucesso!");
    }
}